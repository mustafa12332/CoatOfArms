...
namespace RTSEngine
{
	public class APC : MonoBehaviour {
        ...
        public List<string> UnitExceptionList = new List<string>(); //a list of the unit codes that are not allowed to get in the APC
		...
		//removing all units:
		public bool EjectAllOnly = true; //true when the APC is allowed to eject units all at once
		public int EjectAllOnlyTaskCategory = 0; //the category ID of ejecting all units at once
		...
		public void CallForUnits ()
		{
			...
			while (i < FactionMgr.Units.Count && CurrentUnits.Count < MaxAmount) { //go through the faction's units while still making sure that there is more space for units to get in
				...
				if (!FactionMgr.Units [i].gameObject.GetComponent<APC>() && (!UnitExceptionList.Contains (FactionMgr.Units [i].Code) || AllowAllUnits == true) && FactionMgr.Units [i].gameObject.activeInHierarchy == true) {
