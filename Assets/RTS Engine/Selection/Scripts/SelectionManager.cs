...
namespace RTSEngine
{
	public class SelectionManager : MonoBehaviour {
		...
		void Update () 
		{
			...
			if (BuildingPlacement.IsBuilding == false) { //If we are not placing a building
					...
					if (Input.GetMouseButtonDown (1) || Input.GetMouseButtonDown (0)) {
						...
						if (Physics.Raycast (RayCheck, out Hit, 80.0f, RaycastLayerMask.value)) {  
							...
							if (HitObj != null) {
                                ...
                                HitRpg = HitObj.MainObj.GetComponent<stats>();
							...
							if (Input.GetMouseButtonDown (1)) {
								if (UnitMgr.AwaitingTaskType != TaskManager.TaskTypes.Null) { //if we click with the right mouse button while having an awaiting component task..
                                    ...
                                    if (HitObj != null)
                                    {
                                        ...
                                        else if (HitUnit != null)
                                        {
                                            ...
                                            ActionOnRpg(HitObj, TaskManager.TaskTypes.Null);
                                    }
                                    else
                                    {
										...
										if (SelectedBuilding != null) {
											...
											if (SelectedBuilding.IsBuilt == true) {
												...
												if (SelectedBuilding.Rallypoint != null && SelectedBuilding.FactionID == GameManager.PlayerFactionID) {
													if (HitObj == null && TerrainMgr.IsTerrainTile(Hit.transform.gameObject)) { //if the player clicked on an empty point in the map's terrain
														...
														if (HitBuilding) { //if it's a building
															...
															if (HitBuilding.FactionID == GameManager.PlayerFactionID) {
																...
																HitBuilding.BuildingPlane.GetComponent<Renderer> ().material.color = FriendlyFlashColor;
																HitBuilding.FlashTime = FlashTime;
																HitBuilding.InvokeRepeating ("SelectionFlash", 0.0f, FlashRepeatTime);
															}
														} else if (HitResource) {
															if (HitResource.FactionID == GameManager.PlayerFactionID) {
																...
																HitResource.ResourcePlane.GetComponent<Renderer> ().material.color = FriendlyFlashColor;
																HitResource.FlashTime = FlashTime;
																HitResource.InvokeRepeating ("SelectionFlash", 0.0f, FlashRepeatTime);
															}
														}
													}
												}
								...
								if (((MultipleSelectionKeyDown == false || HitUnit == null) && UnitMgr.AwaitingTaskType == TaskManager.TaskTypes.Null)&& !isBuilding) {
									DeselectUnits ();
								}
                                DeselectRpg();
								DeselectBuilding ();
        ...
        private void ActionOnRpg(SelectionObj hitObj, TaskManager.TaskTypes @null)
        {

            if(SelectedUnits.Count>0)
            {
                if(SelectedUnits[0].team!=hitObj.rpgManager.Team)
                Debug.Log("entered actiononrpg");
                MvtMgr.LaunchAttack(SelectedUnits, hitObj.gameObject, MovementManager.AttackModes.Full);
                hitObj.rpgManager.FlashTime = 2;
                hitObj.rpgManager.rpgPlane.GetComponent<Renderer>().material.color = EnemyFlashColor;
                hitObj.rpgManager.InvokeRepeating("SelectionFlash", 0.0f, FlashRepeatTime);
			...
			if (SelectedUnits.Count > 0) { //Also make sure, at least a unit has been selected
                ...
                { //Units from the player team can be moved by the player, others can't.
                    ...
                    if (PortalUsed == false) //if there was no portal or there was a portal and the units could not use it
                    {
                        ...
                        { ///If the units and the building are from the same team.
                          //If the selected units can construct and the building needs construction & the task type is the build one
                            ...
                            else if (HitBuilding.GetComponent<APC>() && (TaskType == TaskManager.TaskTypes.Null || TaskType == TaskManager.TaskTypes.Mvt))
                            {
                                ...
                                { //loop through the selected units as long as the APC still have space
                                    if (!SelectedUnits[i].gameObject.GetComponent<APC>() && (!CurrentAPC.UnitExceptionList.Contains(SelectedUnits[i].Code) || CurrentAPC.AllowAllUnits == true) && SelectedUnits[i].CanBeMoved == true)
                        ...
                        else if (TaskType == TaskManager.TaskTypes.Null || TaskType == TaskManager.TaskTypes.Attack)
                        {
                            ...
                            if (HitBuilding.Health > 0 && HitBuilding.CanBeAttacked == true)
					...
					if (GameManager.PlayerFactionID == SelectedUnits [0].FactionID) { //Units from the player team can be moved by the player, others can't.
						if(HitResource.FactionID == GameManager.PlayerFactionID || HitResource.CollectOutsideBorder == true)
						{
							if (HitResource.Amount > 0) { //If the selected units can gather resources, and make sure that the player can actually pick this up								
								if (HitResource.WorkerMgr.CurrentWorkers < HitResource.WorkerMgr.WorkerPositions.Length) { //Make sure that there's still room for another collectors:
									...
									int TempBuilderCounter = HitResource.WorkerMgr.CurrentWorkers;
									while (i < SelectedUnits.Count && MaxCollectorsReached == false) { //loop through the selected as long as the max collectors amount has not been reached.
										if (SelectedUnits [i].ResourceMgr && SelectedUnits [i].CanBeMoved == true) { //check if this unit has a gather resource comp (can actually build).
											...
											if (SelectedUnits [i].IsInvisible == false || (SelectedUnits [i].IsInvisible == true && SelectedUnits [i].InvisibilityMgr.CanCollect)) {
												...
												if (TempBuilderCounter < HitResource.WorkerMgr.WorkerPositions.Length) {
		...
		public void ActionOnUnit (Unit HitUnit, TaskManager.TaskTypes TaskType)
		{
			if (SelectedUnits.Count > 0) { //Also make sure, at least a unit has been selected
				...
				if (GameManager.PlayerFactionID == SelectedUnits [0].FactionID) { //Units from the player team can be moved by the player, others can't.
					if (HitUnit.FactionID != SelectedUnits [0].FactionID) { //If the target unit is not dead and has different team I
						if (HitUnit.Dead == false) {
							...
							if (HitUnit.CanBeConverted == true && (TaskType == TaskManager.TaskTypes.Null || TaskType == TaskManager.TaskTypes.Convert) && SelectedUnits [0].ConvertMgr != null) { //if the pending comp task is a convert one
				...
				if (ShowFriendlySelection == true) {
					...
					HitUnit.FlashTime = FlashTime;
					HitUnit.InvokeRepeating ("SelectionFlash", 0.0f, FlashRepeatTime);
				}
			}
		}

		//Resource selection:
        public void selectRpg(stats rpgManager)
        {
            if (SelectedRpg != null) DeselectRpg();
            DeselectUnits();
            DeselectBuilding();
            DeselectResource();

            if(rpgManager.rpgPlane)
            {
                rpgManager.rpgPlane.SetActive(true);
                rpgManager.rpgPlane.GetComponent<Renderer>().material.mainTexture = ResourceSelectionTexture;
                Color SelectionColor = ResourceMgr.ResourceSelectionColor;
                rpgManager.rpgPlane.GetComponent<Renderer>().material.color = new Color(SelectionColor.r, SelectionColor.g, SelectionColor.b, 0.5f);

            }
            UIMgr.UpdateRpgUI(rpgManager);
            SelectedRpg = rpgManager;

        }

        private void DeselectRpg()
        {
            if (SelectedRpg != null)
            {
            ...
            if (SelectedRpg != null)
            {
                GameMgr.Events.OnRpgDeselected(SelectedRpg);
                if (SelectedRpg.rpgPlane)
                    SelectedRpg.rpgPlane.SetActive(false);
            }
            SelectedRpg = null;
        }
