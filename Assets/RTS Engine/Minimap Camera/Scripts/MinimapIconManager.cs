        ...
        public void AssignIconColor (SelectionObj Selection)
        {
            ...
            switch (Selection.ObjType)
            {
                case SelectionObj.ObjTypes.Unit: //in case it's a unit
                    if (Selection.UnitMgr.FreeUnit == false) //if the unit belongs to a faction
                    {
                        //set the faction color
                        IconRenderer.material.color = GameManager.Instance.Factions[Selection.UnitMgr.FactionID].FactionColor;
                    }
                ...
                case SelectionObj.ObjTypes.Building: //If the object to select is a building:
                    ...
                    if (Selection.BuildingMgr.FreeBuilding == false) //building belongs to a faction
                    {
                        //set as the faction color
                        IconRenderer.material.color = GameManager.Instance.Factions[Selection.BuildingMgr.FactionID].FactionColor;
                    }
                    else //free building
                    {
                ...
                case SelectionObj.ObjTypes.Resource: //in case it's a resource
                    ...
                    IconRenderer.material.color = GameManager.Instance.ResourceMgr.ResourcesInfo[Selection.ResourceMgr.ResourceID].MinimapIconColor;
                    break;
                default:
                    break;
            }
