...
namespace RTSEngine
{
    ...
    public enum InputCustomMode { Event, MultipleAttacks, Convert, Invisibility, APCDrop, BuildingResearch, UnitEscape } //a custom input target mode when the input source mode is a custom command
    ...
    public class InputManager : MonoBehaviour
    {
        ...
        public GameObject team1Rpg;
        public GameObject team2Rpg;
        //UNET:
            ...
            if (NetworkType == NetworkTypes.UNET) //if we're using UNET
            {
                if (UNET_Mgr != null) //by checking if there's a valid UNET Faction Manager, we check if the multiplayer game is ready and that the faction has successfully initiliazed.
                {
                    ...
                    if (Input.Target)
                    {
                        ...
                        Debug.Log(Input.Target.name);
                        Input.TargetID = InputManager.Instance.SpawnedObjects.IndexOf(Input.Target);
                        if (Input.TargetID < InputManager.Instance.SpawnedObjects.Count)
                        {
                            //if the objects don't match:
                            try
                            {
        ...
        public void LaunchCommand(InputVars Command)
        {
            ...
            { //spawning the faction capital building
                ...
                int FactionTypeID = GameMgr.GetFactionTypeID(GameMgr.Factions[Command.FactionID].Code);

                if (GameMgr.Factions[Command.FactionID].playerType == "strategy")
                {
                    if (FactionTypeID >= 0 && GameMgr.FactionDef[FactionTypeID].CapitalBuilding != null && GameMgr.Factions[Command.FactionID].team == 0)
                    { //if the faction belongs to a certain type

                        //set the new capital building:
                        Capital = Instantiate(GameMgr.FactionDef[FactionTypeID].CapitalBuilding.gameObject, new Vector3(100, 0, 103.24f), Quaternion.identity);
                    }
                    else
                    {
                        Capital = Instantiate(GameMgr.FactionDef[FactionTypeID].CapitalBuilding.gameObject, new Vector3(14, 0, 26), Quaternion.identity);

                    }

                    //set the capital faction settings
                    ...
                    if (!GameMgr.IsLocalPlayer(Capital))
                    {
                        FogOfWarExplorer explorer = Capital.GetComponent<FogOfWarExplorer>();
                        Destroy(explorer);
                        Capital.gameObject.AddComponent<FogOfWarStalker>();
                    }
                }
                else
                {
                  //  team1Rpg = Instantiate(team1Rpg, new Vector3(17, 0, 29), Quaternion.identity);
                   // InputManager.Instance.SpawnedObjects.Add(team1Rpg); //add the new object to the list
                }
            ...
            { //object creation
                GameObject NewObj = Instantiate(InputManager.Instance.SpawnablePrefabs[Command.SourceID], Command.InitialPos, Quaternion.identity);
                 ...
                 * */
                    ...
                    //building is placed
                    if (GameMgr.IsLocalPlayer(NewObj))
                    {
                        FogOfWarExplorer fogExplorer = NewObj.gameObject.AddComponent<FogOfWarExplorer>();
                        fogExplorer.radius = 15;
                    }
                    else
                        NewObj.gameObject.AddComponent<FogOfWarStalker>();
            ...
            else if (Command.SourceMode == (byte)InputSourceMode.CustomCommand)
            {
                if (GameMgr.Events)
                {
                    ...
                    switch (Command.TargetMode)
                    {
                        ...
                        case (byte)InputCustomMode.BuildingResearch:
                            Source.GetComponent<Building>().LaunchResearchTaskLocal(Command.Value);
                            break;
                            //Unit escaping
            ...
            else if (Command.SourceMode == (byte)InputSourceMode.Unit)
            {
                ...
                { //if there's a target object:
                    ...
                    else if (Command.TargetMode == (byte)InputTargetMode.Unit)
