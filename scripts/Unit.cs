	...
	public class Unit : MonoBehaviour {
		...
		void Awake () {
			...
			if (NavAgent != null) {
				...
				if (FlyingUnit == true && TerrainMgr.AirTerrain != null) {
					NavAgent.baseOffset = TerrainMgr.AirTerrain.transform.position.y * 2;
				}
			}
		...
		void Start ()
		{
           ...
           if( GameMgr.IsLocalPlayer(gameObject) && CreatedBy != null)
            {
               FogOfWarExplorer fogExplorer = gameObject.AddComponent<FogOfWarExplorer>();
                fogExplorer.radius = 15;
            }
            else
            {
                gameObject.transform.GetChild(0).gameObject.AddComponent<FogOfWarStalker>();
            }
            ...
            if (FactionColorObjs.Length > 0)
            {
                ...
                for (int i = 0; i < FactionColorObjs.Length; i++)
                {
                    ...
                    if (FactionColorObjs[i] != null)
                    {
                        ...
                        if (GameMgr.Factions[FactionID].FactionColor.r == 1)
                            FactionColorObjs[i].material = redTeamColor;
                        else
                            FactionColorObjs[i].material = blueTeamColor; ;
        ...
        {
            CheckUnitPathLocal(TargetPos, null,TargetPos, MvtMgr.MvtStoppingDistance, InputTargetMode.None); //move the unit
            NavAgent.speed = EscapeSpeed; //set the escape speed

            //set the escape animator override:
            if (EscapeAnimOverride != null)
            {
                AnimMgr.runtimeAnimatorController = EscapeAnimOverride;
            }
        }
        ...
        void Update () 
		{
            ...
            {
                if (CanRotate == true && Moving == false && CanBeMoved == true)
                {
                    //Unit rotation:
                ...
                { //Is the unit currently moving?
                    ...
                    else
                    {
                        ...
                        { 
                            ...
                            DestinationReached = Vector3.Distance(transform.position, NavAgent.destination) <= NavAgent.stoppingDistance;
                        }
                    }
		...
		public void StopMvt ()
		{
            ...
            if (TargetPosColl)
                TargetPosColl.transform.position = transform.position;
				...
				if (ResourceMgr.TargetResource != null) {
					...
					if (GameManager.MultiplayerGame == false) {
						...
						if (FactionID != GameManager.PlayerFactionID && FreeUnit == false) {
							...
							if (ResourceMgr.TargetResource.WorkerMgr.CurrentWorkers == 1) { //if this unit is the last collector for the target resource.
								if (FactionMgr.ResourceMgr.ExploitedResources [GameMgr.ResourceMgr.GetResourceID (ResourceMgr.TargetResource.Name)].Contains (ResourceMgr.TargetResource)) {
									//remove it then:
		...
		public void CancelAttack ()
		{
			if (AttackMgr != null) {
                ...
                AttackMgr.TargetRpg = null;
                AttackMgr.TargetUnit = null;
                AttackMgr.TargetBuilding = null;
				//Inform the animator that we're returning to the idle state as we stopped this action:
				SetAnimState(UnitAnimState.Idle);
			}
		}
            ...
            if (Health <= 0.0f) {
				...
				if (Dead == false) {
                    ...
                    if (Source != null)
                    {
                        ...
                        {
                            SourceFactionID = Source.gameObject.GetComponent<stats>().FactionId;
                        }
                ...
                if (HealthPoints < 0)
                {
                    ...
                    if (Source != null) //if the attack source is known
                    {
                        ...
                        if (AttackMgr != null)
                        {
                            ...
                            {
                                if (Source.GetComponent<stats>() != null)
                                {
                                    MvtMgr.LaunchAttack(this, Source.gameObject, MovementManager.AttackModes.Change);
                                }
                                else { 
                                //attack back if the unit does not have a target already:
                                if (AttackMgr.AttackTarget == null)
                                {
                                    MvtMgr.LaunchAttackLocal(this, Source.gameObject, MovementManager.AttackModes.Change);
                                }
                                }
                        ...
                        //if singleplayer game or this is the local player:
                        if (GameManager.MultiplayerGame == false || GameMgr.IsLocalPlayer(gameObject) == true)
                        { //if this is an offline game or online but this is the local player
                            if (EnableTakeDamageAnim == false)
                        ...
                        if (GameManager.MultiplayerGame == false)
                        {
                            ...
                            if (FactionID != GameManager.PlayerFactionID && FreeUnit == false)
                            {
                                ...
                                if (Center != null)
                                {
                                    ...
                                    {
                                        //Ask for support:
                                        Collider[] UnitsInRange = Physics.OverlapSphere(transform.position, FactionMgr.ArmyMgr.AttackingSupportRange, MvtMgr.UnitLayerMask);
        ...
        public void ConvertUnitLocal (Unit Converter)
		{
            ...
            if(!GameMgr.IsLocalPlayer(gameObject))
            {
                PlayerSelection.MinimapIcon.gameObject.AddComponent<FogOfWarStalker>();
            }
