...
namespace RTSEngine
{
    public class MovementManager : MonoBehaviour {
        ...
        public LayerMask UnitLayerMask; //a layer mask that contains unit-related layers.
            ...
            { //make sure, at least a unit has been selected
              //sort the units by their types
                ...
                foreach (UnitTypes UnitGroup in SelectedUnitTypes)
                {
                    ...
                    {
                        //mvt order audio:
                        if(GameMgr.GeneralAudioSource)
                        AudioManager.PlayAudio(Camera.main.gameObject, CurrentUnitsList[0].MvtOrderAudio, false);
            ...
            if (GameManager.MultiplayerGame == true)
            {
                //and this is the owner of the unit
                    ...
                    if (TargetObj.GetComponent<stats>()) { 
                        if (!TargetObj.GetComponent<stats>().DeadFlag && RefUnit.team != TargetObj.GetComponent<stats>().Team)
                    {
            ...
            if (SelectedUnitTypes.Count > 0)
            {
                ...
                foreach (UnitTypes UnitGroup in SelectedUnitTypes)
                {
                    ...
                    else if (TargetObj.GetComponent<Building>())
                    {
                        ...
                        CircleRadius = TargetObj.GetComponent<CharacterController>().radius +  AttackManager.Instance.RangeTypes[UnitGroup.RangeTypeID].BuildingStoppingDistance;
                            ...
                            if (AssignedAttackOnly == false || CurrentUnitsList[i].AttackMgr.AttackOnAssign == true)
                            {
                                ...
                                if (CurrentUnitsList[i].IsInvisible == false || CurrentUnitsList[i].InvisibilityMgr.CanAttack == true)
                                {
                                    ...
                                    if (CurrentUnitsList[i].AttackMgr.AttackTarget == null || ChangeTarget == true)
                                    {
                                        ...
                                        if (AttackManager.Instance.CanAttackTarget(CurrentUnitsList[i].AttackMgr, TargetObj, CurrentUnitsList[i].AttackMgr.AttackExceptionList) == true)
                                        {
                                            //get a valid position for the unit to move to
                                            while (Positions.Count == 0)
                                            {
                ...
                if (MainAttackUnit)
                {
                    if (MainAttackUnit.FactionID == GameManager.PlayerFactionID)
                    {
                        ...
                        {
                            TargetObj.GetComponent<Building>().BuildingPlane.GetComponent<Renderer>().material.color = SelectionMgr.EnemyFlashColor;
                            TargetObj.GetComponent<Building>().FlashTime = SelectionMgr.FlashTime;
                            TargetObj.GetComponent<Building>().InvokeRepeating("SelectionFlash", 0.0f, SelectionMgr.FlashRepeatTime);
                        }
                        else if (TargetObj.GetComponent<Unit>())
                        {
                            TargetObj.GetComponent<Unit>().UnitPlane.GetComponent<Renderer>().material.color = SelectionMgr.EnemyFlashColor;
                            TargetObj.GetComponent<Unit>().FlashTime = SelectionMgr.FlashTime;
                            TargetObj.GetComponent<Unit>().InvokeRepeating("SelectionFlash", 0.0f, SelectionMgr.FlashRepeatTime);
                        }
                    }
                }
            }
        ...
        public bool IsDestinationClear(Vector3 Destination, Unit RefUnit, out Vector3 MvtPos)
        {
            ...
            //check if there
            Collider[] UnitsInRange = Physics.OverlapSphere(Destination, RefUnit.NavAgent.radius, UnitLayerMask);
