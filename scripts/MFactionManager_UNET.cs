...
namespace RTSEngine
{
    public class MFactionManager_UNET : NetworkBehaviour {
        ...
        [ClientRpc]
        public void RpcCheckPlayerType(int team,int factionId)
        {
            ...
            foreach (PlayerController player in players)
            {
                ...
                {
                    player.gameObject.GetComponent<stats>().facId = factionId;
                    player.gameObject.GetComponent<stats>().Team = team;
                }
            }
            if (FactionID == GameManager.PlayerFactionID)
                ...
                {
                    rpgCamera = Camera.main;

                    if (Camera.main.GetComponent<ThirdPersonCamera>() == null)
                    {
                        Camera.main.gameObject.SetActive(false);
                        Destroy(rpgCamera.gameObject.GetComponent<CameraMovement>());
                        Destroy(rpgCamera.gameObject.GetComponent<FogOfWarEffect>());
                        rpgCamera.gameObject.AddComponent<ThirdPersonCamera>();
                    }

                   rpgCamera.gameObject.SetActive(true);
                  ...
                  foreach (Transform child in childs)
                    ...
                    GameObject strCanvas = GameObject.FindGameObjectWithTag("strategyCanvas");
                    foreach(Transform child in strCanvas.transform)
                    {
                        if(child.gameObject.tag != "pauseGame")
